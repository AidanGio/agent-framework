...
`create_deep_agent` always wires certain tools (e.g. `task` for subagent
dispatch) when their backing middleware is present. When a configuration
doesn't need them, this middleware drops the named tools from the request
before the model sees them. Mirrors the behavior of deepagents' own private
`_ToolExclusionMiddleware` but lives here so we don't depend on a private
import path.
"""
