...
Checks the LangGraph store for pending messages that arrived while the agent
was busy and injects them as new human messages before the next model call.
"""
...
class QueuedMessageState(AgentState):
    """Extended agent state for tracking out-of-band notifications."""
...
@before_model(state_schema=QueuedMessageState)
async def check_message_queue_before_model(  # noqa: PLR0911
    state: QueuedMessageState,  # noqa: ARG001
